...
class CacheManager {
                ...
                this.fetchCatalogPromise = (async () => {
                    try {
                        ...
                        if (data && data.length > 0) {
                            ...
                            this.catalogString = data.map(s => `- ${s.title}: (Duración: ${s.duration_minutes || 60} mins) - Precio: ${s.price || 'Por cotizar'}`).join('\n');
                        }
                        else {
                            this.catalogArray = [];
                    ...
                    try {
                        ...
                        if (siteRes.data) {
                            siteRes.data.forEach(item => {
                                ...
                                if (key === 'site_name')
                                    configData.siteName = item.value || configData.siteName;
                                if (key === 'agenda_open')
                                    configData.open = item.value || configData.open;
                                if (key === 'agenda_close')
                                    configData.close = item.value || configData.close;
                                if (key === 'agenda_buffer')
                                    configData.buffer = parseInt(item.value) || configData.buffer;
                                if (key === 'agenda_concurrency')
                                    configData.concurrency = parseInt(item.value) || configData.concurrency;
                        ...
                        if (aiRes.data) {
                            aiRes.data.forEach(item => {
                                ...
                                if (key === 'business_hours')
                                    configData.business_hours_text = item.value;
                                if (key === 'closed_days')
                                    configData.closed_days_text = item.value;
                                if (key === 'agent_name')
                                    configData.agentName = item.value;
                                if (key === 'agent_personality')
                                    configData.agentPersonality = item.value;
                                if (key === 'business_rules')
                                    configData.businessRules = item.value;
                                if (key === 'bot_system_prompt')
                                    configData.masterPrompt = item.value;
                                // Lógica prioritaria
                                if (key === 'agenda_open')
                                    configData.open = item.value || configData.open;
                                if (key === 'agenda_close')
                                    configData.close = item.value || configData.close;
                                if (key === 'simultaneous_appointments')
                                    configData.concurrency = parseInt(item.value) || configData.concurrency;
                                if (key === 'agenda_buffer')
                                    configData.buffer = parseInt(item.value) || configData.buffer;
                                if (key === 'appointment_duration_minutes')
                                    configData.defaultDuration = parseInt(item.value) || configData.defaultDuration;
                                if (key === 'max_daily_appointments')
                                    configData.maxDaily = parseInt(item.value) || 8;
                                if (key === 'agenda_closed_days') {
                                    if (item.value && item.value.trim() !== "") {
                                        configData.closedDays = item.value.split(',').map(d => parseInt(d.trim()));
                                    }
                        ...
                        logger.error("Error fetching agenda config", { error });
    ...
    invalidateConfig() {
        ...
        this.lastCatalogFetch = 0;
        this.lastAgendaConfigFetch = 0;
        logger.info("Cache de configuración invalidado.");
    }
}
