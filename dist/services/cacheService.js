...
async function getAgendaConfig() {
    if (Date.now() - lastAgendaConfigFetch > config.CACHE_TTL.AGENDA_CONFIG || !agendaConfigCache) {
        ...
        if (data) {
            data.forEach(item => {
                if (item.key === 'agenda_open')
                    configAgenda.open = item.value || config.BUSINESS_HOURS.OPEN;
                if (item.key === 'agenda_close')
                    configAgenda.close = item.value || config.BUSINESS_HOURS.CLOSE;
                if (item.key === 'agenda_buffer')
                    configAgenda.buffer = parseInt(item.value) || config.BUSINESS_HOURS.BUFFER;
                if (item.key === 'agenda_concurrency')
                    configAgenda.concurrency = parseInt(item.value) || config.BUSINESS_HOURS.CONCURRENCY;
